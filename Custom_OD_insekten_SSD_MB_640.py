...
get_ipython().system('unzip Custom_OD_640')
...
TRAIN_IMAGE_FILE = '/Custom_OD/Workspace/images/Test'
import glob
...
get_ipython().run_line_magic('tensorboard', '--logdir /content/Custom_OD/Workspace/models/ssd_mobilenet_v2_fpnlite_640x640_coco17_tpu-8')


# Using the batch_size of 5 and num_steps to be 30000. Change these parameters in the pipeline.config file in the models/ssd_... folder

# In[10]:


get_ipython().run_cell_magic('bash', '', 'cd Custom_OD/Workspace/\npython model_main_tf2.py --model_dir=/content/Custom_OD/Workspace/models/ssd_mobilenet_v2_fpnlite_640x640_coco17_tpu-8 --pipeline_config_path=/content/Custom_OD/Workspace/models/ssd_mobilenet_v2_fpnlite_640x640_coco17_tpu-8/pipeline.config')


# In[11]:


get_ipython().run_cell_magic('bash', '', 'cd Custom_OD/Workspace/\npython exporter_main_v2.py --input_type=image_tensor --pipeline_config_path=/content/Custom_OD/Workspace/models/ssd_mobilenet_v2_fpnlite_640x640_coco17_tpu-8/pipeline.config --trained_checkpoint_dir=/content/Custom_OD/Workspace/models/ssd_mobilenet_v2_fpnlite_640x640_coco17_tpu-8 --output_directory=/content/Custom_OD/Workspace/exported_model/ssd_mb_640')
...
PATH_TO_SAVED_MODEL =r'/content/Custom_OD/Workspace/exported_model/ssd_mb_640/saved_model'
...
# Now let us run the inference on the test set with the previously trained ssd_mb_320 model on Kaggle Dataset and see it that model is able to generalize.
...
#Image files for inference
PATH_TO_TEST_IMAGES_DIR = pathlib.Path('/content/Custom_OD/Workspace/images/Test_Kaggle')
TEST_IMAGEs_PATH = sorted(list(PATH_TO_TEST_IMAGES_DIR.glob("*.jpg")))
for image_path in TEST_IMAGEs_PATH:
  print(image_path)
  show_inference(detect_fn, image_path)
